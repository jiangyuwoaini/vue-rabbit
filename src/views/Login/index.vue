...
// 3.获取 form 实例做统一校验,防止其他校验不生效,通过触发调用校验
...
const doLogin = () => {
  ...
  formRef.value.validate(async (valid) => {
    ...
    if (valid) {
      // TODO LOGIN 
      await userStore.getUserInfo({ account, password }) //用户名 13888888888 密码 123456
      // 1. 提示用户    
          ...
          <div class="form">
            <el-form ref="formRef" label-position="right" :model="form" :rules="rules" label-width="60px" status-icon>
              <el-form-item prop="account" label="账户">
                <el-input v-model="form.account" /> <!-- 用v-model进行表单的双向绑定-->
              </el-form-item>
